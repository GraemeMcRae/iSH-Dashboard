...
I made a one-click dashboard using iSH. The .profile has three commands: ssh to our Linux server, sleep 15, then exit. The Linux server ~/.batchrc displays the dashboard, sleeps for 10 minutes then exits, causing the ssh session to drop. Rinse and repeat.
